...
def AbstandzumVordermann(Autobahn, Index):
    a = len(Autobahn)
    counter = 0
    for i in range(1, a + 1):
        if Autobahn[(Index + i) % a][0] == 1:
            counter += 1
            return (counter)
        else:
            counter += 1
    ...
    for i in range(len(Autobahn)):
        if Autobahn[i][0] == 1:
            ...
            if b < Autobahn[i][1]:  # Sicherheitsabstand
                Autobahn[i][1] = b
            if (i + Autobahn[i][1]) >= len(Autobahn):
                neueS[(i + Autobahn[i][1]) % len(Autobahn)][0] = 1
                neueS[(i + Autobahn[i][1]) % len(Autobahn)][1] = Autobahn[i][1]
                continue
            else:
...
def Höchstgeschwindigkeit(Dichte):
    ...
    if Dichte == 0:
        ...
        return ((int((1 / Dichte) - 2 )))

def neueGeschwindigkeitsbegrenzung(Autobahn):
    ...
    return ([Höchstgeschwindigkeit(b), Höchstgeschwindigkeit(c), Höchstgeschwindigkeit(d), Höchstgeschwindigkeit(e), Höchstgeschwindigkeit(f), Höchstgeschwindigkeit(g), Höchstgeschwindigkeit(a)])
